...
| P03 | 로컬 Ollama 벤치마크 | 진행 | 공식 `0.32.9` 비root Docker 서비스와 전용 모델 구성, 강화 짧은 v2/v3 시험 완료 | v3 97.43초로 기존 속도 기준 실패 | 수집 완료 데이터를 사용해 묶음 요약 방식 재검증 |
...
| P06 | 버전 비교·자동 묶음 | 재작업 | 버전 범위·메이저/minor·beta/RC·지원상태/EOL·변경유형 후보 구현 | 주제별 묶음·중복 통합·영향도 선별 미구현 | 원문을 변경하지 않는 요약 입력 묶음 구현 |
| P07 | AI 요약 및 OmniRoute | 진행 | 라우팅 순서와 로컬 모델 초기 성능 확인 | 고객용·DBA용 자동 요약과 근거 연결 미구현 | 수집된 실제 데이터로 Ollama 요약 검증 후 fallback 구현 |
| P08 | 최종 결과 검수 화면 | 재작업 | 원문 비교·필터·항목별 승인/반려·수정 이력 구현 | 항목별 수동 검수가 기본이라 사용자 업무를 줄이지 못함 | 완성된 보고서 초안 중심 검토·수정·확정 흐름으로 변경 |
| P09 | 요약 보고서 출력 | 재작업 | 5종 출력과 고객·프로젝트·로고, 한글 문서 렌더링 기반 구현 | 개별 승인 항목이 출력 원천이라 목적과 불일치 | 최종 확정 report revision을 5종 출력의 원천으로 변경 |
| P10 | 운영과 복구 | 진행 | 외부경로 강제 백업·체크섬, 격리된 빈 환경 전체 복원, health timer와 VM 재부팅 자동복구 검증 | 실제 외부 저장소와 외부 알림 미정 | 외부 저장소·알림 채널 확정 |
| P11 | 시범 검증과 출시 | 대기 | 대표 범위의 원문 구조 검증과 기존 문서 렌더링 검증 완료 | 핵심 자동 요약·최종 초안 흐름이 아직 미구현 | P06~P09 재작업 후 요약 정확도와 사용자 작업량 검증 |

### 2026-08-13 / 계획 정정 / 원본 불변과 자동 요약 중심 흐름

- 사용자가 수집 원문은 원본 그대로 보관하고 가공 결과로 덮어쓰면 안 된다고 재확인.
- `source_snapshot`을 불변 원본으로 정의하고 구조화·묶음·AI 요약·사람 수정·최종 보고서를 별도 데이터로 저장하도록 계획 수정.
- 공식 원문 변경은 기존 행 수정이 아니라 새 해시 스냅샷 추가 방식으로만 처리.
- 데이터 수집과 범위 비교가 완료됐으므로 AI 작업을 더 미루지 않고 P03·P07을 현재 단계로 이동.
- 기존 항목별 승인·반려는 기본 사용자 흐름으로 부적합하다고 판정하고 예외 조사용 보조 기능으로 변경 예정.
- P08은 자동 생성된 최종 보고서 초안 검토·수정·확정 화면으로, P09는 확정 revision 출력으로 재작업.
- P11 출시는 P06~P09 재작업과 실제 요약 정확도 검증 이후로 되돌림.
...
### 2026-08-13 / P11 / 대표 구간 검증과 보고서 한계 고지

구현:

- `validate_pilot_ranges` 관리 명령으로 대표 업그레이드 구간을 반복 검증 가능하게 구성.
- 각 범위의 릴리스·섹션·변경항목 집계와 현재 스냅샷 수를 교차 확인.
- 공식 PostgreSQL URL, 파서 v2, 항목 raw HTML SHA-256, raw HTML 재추출 텍스트 일치를 검증.
- 모든 보고서 형식에 공식 릴리스 노트 기반 자료라는 한계, 별도 호환성 시험, 백업·복구 및 실제 데이터 리허설 필요성을 표시.
- DOCX 항목 본문과 공식 원문 링크를 같은 페이지 블록으로 유지하고 하단 안전 여백을 보완.
- 출시 전 남은 조건을 `docs/RELEASE_CHECKLIST.md`로 분리해 관리.

대표 구간 결과:

```text
9.2.10 → 18.4: 286 releases / 13 major / 273 minor / 1,219 sections / 13,110 items
12.0 → 16.0: 90 releases / 4 major / 86 minor / 374 sections / 4,476 items
14.0 → 18.0: 69 releases / 4 major / 65 minor / 308 sections / 3,546 items
17.0 → 18.0: 11 releases / 1 major / 10 minor / 60 sections / 654 items
18.3 → 18.4: 1 release / 0 major / 1 minor / 3 sections / 74 items
오류: 0
```
...
```text
버전: 9.2.10 / 12.0 / 14.0 / 16.0 / 18.4
HTTP: 모두 200
운영 스냅샷과 현재 공식 페이지 SHA-256: 모두 일치
공식 페이지와 구조화 항목 수: 69/69, 186/186, 229/229, 213/213, 74/74
항목 순서와 추출 텍스트: 모두 일치
```
...
```text
Django tests: 32 passed
스냅샷: 346 releases / 346 current snapshots / 15,521 items, 무결성 오류 0
보고서 임시 데이터 정리: 0 profiles / 0 reviews
PDF: 2페이지 전부 한글·로고·한계 고지·링크 배치 정상
DOCX: 3페이지 전부 한글·로고·한계 고지·링크·바닥글 배치 정상
```

판정:

- 대표 데이터 검증과 알려진 한계 표시 조건은 통과.
- 출시 체크리스트의 HTTPS, 실제 Word 데스크톱, 외부 백업·알림, 사용자 승인 조건이 남아 P11은 `진행` 유지.

### 2026-08-13 / P09 / VM IP 기반 내부 접속

- 도메인과 HTTPS는 현재 범위에서 제외.
- 웹 서비스만 VM 주소 `10.65.50.99`의 TCP 80에 공개하도록 구성.
- 접속 주소는 `http://10.65.50.99`로 고정.
- PostgreSQL 5432와 Ollama 11434는 호스트에 공개하지 않고 내부 네트워크 전용으로 유지.
- Docker 포트는 전체 인터페이스가 아닌 사설 IP `10.65.50.99`에만 바인딩.
- VM 내부와 Mac에서 `/health/` HTTP 200 및 DB 연결 정상 확인.
- UFW 규칙 변경은 sudo 대화형 인증이 필요해 수행하지 않았으며, 현재 내부망 접속에는 추가 규칙이 필요하지 않았음.
- 관리자 비밀번호는 최소 8자만 요구하고 문자 종류 조합은 강제하지 않도록 설정.
- `rock` 웹 superuser 생성 후 사용자가 비밀번호 변경 화면에서 직접 새 비밀번호 설정.
- 서버에서 계정 활성·staff·superuser 상태와 임시 비밀번호 폐기를 확인.
